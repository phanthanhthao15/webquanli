<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.1/css/all.min.css" integrity="sha512-MV7K8+y+gLIBoVD59lQIYicR65iaqukzvf/nwasF0nqhPay5w/9lJmVM2hMDcnK1OnMGCdVK+iQrJ7lzPJQd1w==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <link rel="stylesheet" href="../css/style.css">
    <title>Trang Chủ - Quần áo  thời trang</title>
</head>
<body class="preloading">
    <div class="wrapper">
        <header>
            <div class="header-wrapper">
                <div class="box-logo_mail flex-row">
                    <div class="logo">
                        <img src="../image/thanh thảo shop.png" alt="">
                    </div>
                    <div class="phone-mail">
                        <div class="phone">
                            <div class="icon">
                                <i class="fa-solid fa-phone"></i>
                            </div>
                            <div class="title">
                                <span>Tư vấn sản phẩm </span>
                                <p>Phan Thanh Thảo  0822104248<p>
                            </div>
                        </div>
                        <div class="mail">
                            <div class="icon">
                                <i class="fa-regular fa-envelope"></i>
                            </div>
                            <div class="title">
                                <span>Gửi email liên hệ</span>
                                <p>phanthanhthao15012004@gmail.com</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="box-menu">
                    <div class="menu-wrapper">
                        <ul>
                        <li id="trangchu"><a href="index.php">Trang Chủ shop </a></li>
                            <li id="gioithieu"><a href="introduce.php">Giới Thiệu về shop</a></li>
                            <li id="sanpham"><a href="product.php">Sản Phẩm chính</a></li>
                            <li id="installment"><a href="installment.php">phụ kiện thời trang</a></li>                                                     
                            <li id="contact"><a href="contact.php">Đánh giá sản phẩm</a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </header>
        <div class="body_shop">
            <div class="body_shop-wrapper">
                <div class="slideshow">
                    <button class="left"><i class="fa-solid fa-chevron-left"></i></button>
                    <div class="imageToyota">
                        <img src="../image/bìa/b4.jpg" alt="">
                        <img src="../image/bìa/b5.jpg" alt="">
                        <img src="../image/bìa/b3.jpg" alt="">
                    </div>
                    <button class="right"><i class="fa-solid fa-chevron-right"></i></button>
                    <div class="list-slide">
                        <div class="box-list">
                            <span for="radio1" class="active"></span>
                            <span for="radio1"></span>
                            <span for="radio1"></span>
                        </div>
                    </div>
                </div>
                <div class="product1">
                    <div class="title-product">
                        <h1>CELINE</h1>
                    </div>
                    <div class="product-toyota">
                        <button class="leftproduct"><i class="fa-solid fa-chevron-left"></i></button>
                        <div class="product-toyotaxe">
                            <div class="product-item1 product-xe">
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/gg/cl3.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>CELINE made in Italy</h3>
                                        <p>Auth</p>
                                        <del>10,000,000<u>đ</u></del>
                                        <span>8,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/gg/cl1.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>CELINE made in Italy</h3>
                                        <p>Auth</p>
                                        <del>10,000,000<u>đ</u></del>
                                        <span>7,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/gg/cl2.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>CELINE made in Italy</h3>
                                        <p>Auth</p>
                                        <del>10,000,000<u>đ</u></del>
                                        <span>7,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                            </div>
                            <div class="product-item1 product-xe">
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/gg/cl4.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>CELINE made in Italy</h3>
                                        <p>Auth</p>
                                        <del>10,000,000<u>đ</u></del>
                                        <span>7,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/gg/cl5.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>CELINE made in Italy</h3>
                                        <p>Auth</p>
                                        <del>10,000,000<u>đ</u></del>
                                        <span>7,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/gg/cl6.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>CELINE made in Italy</h3>
                                        <p>Auth</p>
                                        <del>10,000,000<u>đ</u></del>
                                        <span>7,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <button class="rightproduct"><i class="fa-solid fa-chevron-right"></i></button>
                    </div>
                </div>
                <div class="product2">
                    <div class="title-product">
                        <h1>HERMES</h1>
                    </div>
                    <div class="product2-toyota">
                        <button class="leftproduct2"><i class="fa-solid fa-chevron-left"></i></button>
                        <div class="product2-toyotaxe">
                            <div class="product-item1 product-xe">
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/lv/AO-PHONG-HERMES-NAM-MAU-DEN-HOA-TIET-CO-VIEN-CAM-LOGO-NGUA-APH08-2.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>HERMES made in Italy</h3>
                                        <p>Auth</p>
                                        <del>14,000,000<u>đ</u></del>
                                        <span>8,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/lv/2.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>HERMES made in Italy/h3>
                                        <p>Auth</p>
                                        <del>14,000,000<u>đ</u></del>
                                        <span>8,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/lv/3.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>HERMES made in France</h3>
                                        <p>Auth</p>
                                        <del>14,000,000<u>đ</u></del>
                                        <span>8,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                            </div>
                            <div class="product-item1 product-xe">
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/lv/4.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>HERMES made in Italy</h3>
                                        <p>Auth</p>
                                        <del>14,000,000<u>đ</u></del>
                                        <span>8,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/lv/5.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>HERMES made in Italy</h3>
                                        <p>Auth</p>
                                        <del>14,000,000<u>đ</u></del>
                                        <span>8,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/lv/6.webp" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>HERMES made in Italy</h3>
                                        <p>Auth</p>
                                        <del>14,000,000<u>đ</u></del>
                                        <span>8,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                            </div>
                            <div class="product-item1 product-xe">
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/lv/7.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>HERMES made in Italy</h3>
                                        <p>Auth</p>
                                        <del>14,000,000<u>đ</u></del>
                                        <span>8,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/lv/8.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>HERMES made made in Italy</h3>
                                        <p>Auth</p>
                                        <del>14,000,000<u>đ</u></del>
                                        <span>8,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/lv/9.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>HERMES made in made in Italy</h3>
                                        <p>Auth</p>
                                        <del>14,000,000<u>đ</u></del>
                                        <span>8,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                            </div>
                            
                        </div>
                        <button class="rightproduct2"><i class="fa-solid fa-chevron-right"></i></button>
                    </div>
                </div>
                <div class="product3">
                    <div class="title-product">
                        <h1>Parada</h1>
                    </div>
                    <div class="product3-toyota">
                        <button class="leftproduct3"><i class="fa-solid fa-chevron-left"></i></button>
                        <div class="product3-toyotaxe">
                            <div class="product-item1 product-xe">
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/bl/1.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Parada made in Italy</h3>
                                        <p>Auth</p>
                                        <del>9,000,000<u>đ</u></del>
                                        <span>5,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/bl/2.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Parada made in Italy</h3>
                                        <p>Auth</p>
                                        <del>9,000,000<u>đ</u></del>
                                        <span>5,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/bl/3.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Parada made in Italy</h3>
                                        <p>Auth</p>
                                        <del>9,000,0000<u>đ</u></del>
                                        <span>5,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                            </div>
                            <div class="product-item1 product-xe">
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/bl/4.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Parada made in Italy</h3>
                                        <p>Auth</p>
                                        <del>9,000,000<u>đ</u></del>
                                        <span>5,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/bl/5.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Parada made in Italy</h3>
                                        <p>Auth</p>
                                        <del>9,000,000<u>đ</u></del>
                                        <span>5,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/bl/6.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Parada made in Italy</h3>
                                        <p>Auth</p>
                                        <del>9,000,000<u>đ</u></del>
                                        <span>5,500,000 <u>đ</u></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <button class="rightproduct3"><i class="fa-solid fa-chevron-right"></i></button>
                    </div>
                </div>
                <div class="product4">
                    <div class="title-product">
                        <h1>Givenchy</h1>
                    </div>
                    <div class="product4-toyota">
                        <button class="leftproduct4"><i class="fa-solid fa-chevron-left"></i></button>
                        <div class="product4-toyotaxe">
                            <div class="product-item1 product-xe">
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/mlb/1.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Givenchy made in Italy</h3>
                                        <p>Auth</p>
                                        <del>7,000,000<u>đ</u></del>
                                        <span>4,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/mlb/2.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Givenchy made in Italy</h3>
                                        <p>Auth</p>
                                        <del>7,000,000<u>đ</u></del>
                                        <span>4,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/mlb/3.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Givenchy made in Italy</h3>
                                        <p>Auth</p>
                                        <del>7,000,000<u>đ</u></del>
                                        <span>4,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                            </div>
                            <div class="product-item1 product-xe">
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/mlb/4.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Givenchy made in Italy</h3>
                                        <p>Auth</p>
                                        <del>7,000,000<u>đ</u></del>
                                        <span>4,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/mlb/5.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Givenchy made in Italy</h3>
                                        <p>Auth</p>
                                        <del>7,000,000<u>đ</u></del>
                                        <span>4,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/mlb/6.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Givenchy made in Italy</h3>
                                        <p>Auth</p>
                                        <del>7,000,000<u>đ</u></del>
                                        <span>4,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <button class="rightproduct4"><i class="fa-solid fa-chevron-right"></i></button>
                    </div>
                </div>
                <div class="product5">
                    <div class="title-product">
                        <h1>Fendi</h1>
                    </div>
                    <div class="product5-toyota">
                        <button class="leftproduct5"><i class="fa-solid fa-chevron-left"></i></button>
                        <div class="product5-toyotaxe">
                            <div class="product-item1 product-xe">
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/dr/1.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Fendi made in Italy</h3>
                                        <p>Auth</p>
                                        <del>13,000,000<u>đ</u></del>
                                        <span>10,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/dr/2.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Fendi made in Italy</h3>
                                        <p>Auth</p>
                                        <del>13,000,000<u>đ</u></del>
                                        <span>10,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/dr/3.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Fendi made in Italy</h3>
                                        <p>Auth</p>
                                        <del>13,000,000<u>đ</u></del>
                                        <span>10,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                            </div>
                            <div class="product-item1 product-xe">
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/dr/4.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Fendi made in Italy</h3>
                                        <p>Auth</p>
                                        <del>13,000,000<u>đ</u></del>
                                        <span>10,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/dr/5.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Fendi made in Italy</h3>
                                        <p>Auth</p>
                                        <del>13,000,000<u>đ</u></del>
                                        <span>10,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                                <div class="productvios">
                                    <div class="image-product">
                                        <img src="../image/dr/6.jpg" alt="">
                                    </div>
                                    <div class="product-title">
                                        <h3>Fendi made in Italy</h3>
                                        <p>Auth</p>
                                        <del>13,000,000<u>đ</u></del>
                                        <span>10,000,000 <u>đ</u></span>
                                    </div>
                                </div>
                            </div>
                            
                        </div>
                        <button class="rightproduct5"><i class="fa-solid fa-chevron-right"></i></button>
                    </div>
                </div>
            </div>
            <div class="youtube-posts">
                <div class="youtubeposts-wrapper">
                    <div class="box-youtube">
                        <div class="youtube-wrapper">
                            <iframe width="400" height="325" src="../image/thanh thảo shop.png" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
                        </div>
                    </div>
                    <div class="box-posts">
                        <div class="posts-content">
                            <div class="image-content">
                                <img src="../image/mau1.jpg" alt="">
                            </div>
                            <div class="content">
                                <h3>Chọn màu áo theo sở thích</h3>
                                <p>Nhiều người cho rằng, việc chọn màu áo phù hợp với màu da của mình ...</p>
                            </div>
                        </div>
                        <div class="posts-content">
                            <div class="image-content">
                                <img src="../image/vai.png" alt="">
                            </div>
                            <div class="content">
                                <h3>Chọn chất liệu vải</h3>
                                <p>Nhiều người cho rằng, việc chọn chất liệu vải sẽ giúp người mặc thấy thoải mái ...</p>
                            </div>
                        </div>
                        <div class="posts-content">
                            <div class="image-content">
                                <img src="../image/nhanhieu.jpg" alt="">
                            </div>
                            <div class="content">
                                <h3>Chọn màu các nhãn hiệu</h3>
                                <p>Nhiều người cho rằng, việc chọn nhãn hiệu tùy sở thích của mỗi người...</p>
                            </div>
                        </div>
                        <div class="posts-content">
                            <div class="image-content">
                                <img src="../image/tien.jpg" alt="">
                            </div>
                            <div class="content">
                                <h3>Chọn mua đắt hay rẻ</h3>
                                <p>Nhiều người cho rằng, việc chọn mua đắt hay rẻ tùy vào tài chính của mỗi người...</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <footer>
                <div class="footer1">
                    <div class="footer1-left">
                        <h1>
                            xí nghiệp Quần Áo Nông Trang
                        </h1>
                        <h3>
                            <b>Xí Nghiệp Quần Áo Nông Trang</b> là doanh nghiệp nhà nước trực thuộc Tổng Công ty TNHH được thành lập ngày 15/11/1999.<br>
                            Địa chỉ: 202, phường Nông Trang, thành phố Việt Trì, Tỉnh Phú Thọ
                        </h3>
                        <p>Hotline Phan Thanh Thảo:0822104248</p>
                    </div>
                    <div class="footer1-right">
                        <div class="left">
                            <h2>giới thiệu</h2>
                            <ul>
                                <li id="gioithieuFooter"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" />
                                    </svg>
                                  <a href="#">Giới thiệu</li>
                                <li><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" />
                                  </svg>
                                  <a href="#">Sản phẩm</a></li>
                                <li><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" />
                                  </svg>
                                  <a href="#">Bảng giá</a></li>
                                <li><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" />
                                  </svg>
                                  <a href="#">Tin tức</a></li>
                            </ul>
                        </div>  
                        <div class="right">
                            <h2>chính sách</h2>
                            <ul>
                                <li><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" />
                                  </svg>
                                  <a href="#">Chính sách bán hàng</a></li>
                                <li id="installmentFooter"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" />
                                  </svg>
                                  <a href="#">Chính sách bảo hành</a></li>
                                <li id="newcarFooter"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" />
                                  </svg>
                                  <a href="#">Giá mặt hàng</a></li>
                                <li><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" />
                                  </svg>
                                  <a href="#">ship toàn quốc</a></li>
                            </ul>
                        </div>                    
                    </div>
                </div>
                <div class="footer2">
                    <div class="footer2-content">
                        <p><i class="fa-solid fa-house-chimney"></i>Phan Thanh Thảo</p>
                        <p><i class="fa-solid fa-phone"></i>Hotline: 0822104248</p>
                        <p><i class="fa-solid fa-gear"></i>Uy tín-Đẹp-Chất lượng</p>
                    </div>
                </div>
            </footer>
        </div>
        <div class="phoneFixed">
            <div class="phone-wrapper">
                <i class="fa-solid fa-phone"></i>
            </div>
        </div>

        <div class="loader-wrapper">
            <div class="loader">
                <div class="inner one" style="--i:1;"></div>
                <div class="inner two" style="--i:1;"></div>
                <div class="inner three" style="--i:1;"></div>
                <div class="box-heart">
                    <div class="heart"></div>
                </div>
            </div>
        </div>
    </div>

    <script src="../js/app.js"></script>
    <script type="text/javascript" src="https://code.jquery.com/jquery-3.6.3.min.js"></script>
    <script>
        $(window).on('load', function(event){
        $('body').removeClass('preloading');
        $('.loader-wrapper').delay(1000).fadeOut('fast');
    });
    </script>
</body>
</html>